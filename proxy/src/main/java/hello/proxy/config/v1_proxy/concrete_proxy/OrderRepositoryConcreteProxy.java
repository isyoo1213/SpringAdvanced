...
public class OrderRepositoryConcreteProxy extends OrderRepositoryV2 {
    ...
    //target 의존성으로 지정 - 부모 클래스를 상속함과 동시에, 부모 클래스의 인스턴스를 주입받음
    private final OrderRepositoryV2 target;
    private final LogTrace tracer;

    public OrderRepositoryConcreteProxy(OrderRepositoryV2 target, LogTrace tracer) {
        this.target = target;
        this.tracer = tracer;
    }
    ...
    public void save(String itemId) {
        ...
        try {
            status = tracer.begin("OrderRepository.save() by Concrete proxy");
